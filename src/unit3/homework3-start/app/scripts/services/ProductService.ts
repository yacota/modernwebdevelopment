...
module auction.service {
    ...
    export class ProductService {
        ...
        public getProductById(productId) : ng.IPromise<m.ProductModel> {
        ...
        private getDataById(urlJSON) : ng.IPromise<m.ProductModel> {
            console.log('ProductService retrieving by Id->'+urlJSON);
            var deferredProducts = this.$q.defer<m.ProductModel>();
            this.$http.get(urlJSON)
                .success((data) => deferredProducts.resolve(<m.ProductModel>data))
                .error(() => deferredProducts.reject());
            return deferredProducts.promise;
        }
    }
